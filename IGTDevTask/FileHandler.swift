...
class FileHandler {
    ...
    private init() {
        ...
        gamesFilePath = documentsDirectory.URLByAppendingPathComponent(gamesFile)
    }
    ...
    func writeDataToFile (data: NSData) {
        ...
        guard fileManager.createFileAtPath(gamesFilePath.absoluteString, contents: data, attributes: nil) else {
            DLog("Cannot create file: \(gamesFilePath.absoluteString)")
            return
        }
        readDataFromFile()
    }
     ...
     */
    func fileValid () -> Bool {
        guard fileManager.fileExistsAtPath(gamesFilePath.absoluteString) else {
            return false
        }
        ...
        do {
            fileAttributes = try fileManager.attributesOfItemAtPath(gamesFilePath.absoluteString)
        } catch {
    ...
    func readDataFromFile () {
        if fileValid(),
           let data : NSData = fileManager.contentsAtPath(gamesFilePath.absoluteString) {
